...
function s_d_build_runtime() {
  sudo env $runtime_platform_env docker compose build "$@"
}
...
if [ -z "$skip_build" ]; then
  ...
  # compile dominates; cross-arch passes also emulate the ES plugin install
  # and the Go build). Cross-arch, when TY_TARGET_ARCH is set — see above.
  ...
  if [ -n "${TY_REUSE_APP_DIST:-}" ] && ls target/universal/talkyard-server-*.zip 1>/dev/null 2>&1; then
    ...
    # unit test results are arch-independent, no need to redo them.
