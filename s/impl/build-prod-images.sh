#!/bin/bash

# Abort on any error
set -e

version_tag="$1"
shift

if [ -z "$version_tag" ]; then
  echo "No version_tag parameter. Usage:  $0 your-username version-tag"
  echo "Bye."
  exit 1
fi

# Docker Compose will mount $HOME/.ivy2 and $HOME/.m2, and we want to mount
# $my_username's Ivy and Maven cache dirs, so change $HOME from /root/ to:
#export HOME=/home/$my_username
# — no longer needed, now building as the same user who starts the script.

echo "
Building: $version_tag
With username: $my_username
And HOME: $HOME
Other args: $@
"


# This'll make us call `exit 1` if there's an error, and we're running all this via a script.
is_in_script=true

# Won't exit if we're doing stuff manually on the command line (because it's annoying
# if the terminal suddenly disappears).
function die_if_in_script {
  if [ -n "$is_in_script" ]; then
    echo "Bye."
    exit 1
  fi
}


# Options?
# ----------------------

all_orig_options="$@"

skip_e2e_tests=''
skip_build=''
skip_restart=''

for arg in "$@"; do
  case $arg in
    #-x=*|--something=*)
    #STH="${i#*=}"
    #hift # past argument=value
    #;
    --skip-e2e-tests)
    echo "Will skip E2E tests, because of --skip-e2e-tests."
    echo
    skip_e2e_tests=yes
    shift
    ;;
    --skip-build)
    echo "Will skip build, because of --skip-build."
    echo
    skip_build=yes
    shift
    ;;
    --skip-restart)
    echo "Will not restart, instead, I'll reuse already running containers,
        because of --skip-restart."
    echo
    skip_restart=yes
    shift
    ;;
    *)
    # Unknown option, ignore. Maybe it's for wdio.conf?
    ;;
  esac
done


# Ensure no other containers running
# ----------------------

# Dupl kill-down prod test code. [KLLPRDTST]  [prod_test_docker_conf]
# Project "tyb1" = Talkyard build, epoch 1.
test_containers='docker compose -p tyb1 -f modules/ed-prod-one-test/docker-compose.yml -f modules/ed-prod-one-test/debug.yml -f modules/ed-prod-one-test-override.yml -f docker-compose-no-limits.yml'

if [ -z "$skip_restart" ]; then
  sudo $test_containers kill web app rendr search cache rdb egressp backup
  sudo $test_containers down

  s/d kill web app
  s/d down
fi

# Any unexpected containers up and running might cause problems.

function containers_running_test() {
  # 'tail -n +2' skips the column titles row. We exclude any '*registry*' container,
  # because it's fine to run a local Docker registry, if testing images on localhost.
  # And we exclude any container started via s/selenium ('tye2ebrowser')
  # or that runs the e2e Bash scripts ('tynodejs').
  sudo docker ps | tail -n +2 | grep -v registry | grep -v tye2ebrowser | grep -v tynodejs
}

if [ -n "`containers_running_test`" ]; then
  if [ -z "$skip_restart" ]; then
    echo
    echo "Docker containers are running, PLEASE STOP THEM. Look:"
    echo
    sudo docker ps
    echo
    die_if_in_script
  else
    echo
    echo "Docker containers are running, will try to use these:  (since --skip-restart)"
    echo
    sudo docker ps
    echo
  fi
fi


# Cross-arch runtime-image builds?
# ----------------------

# TY_TARGET_ARCH (e.g. 'arm64', see `b/build --arch`) makes the RUNTIME image
# builds below target linux/$TY_TARGET_ARCH — the arch-specific layers then
# build via qemu/binfmt emulation. The dev images (nodejs, app-dev) and
# everything that runs in them (gulp, sbt — arch-independent output) stay
# native. DOCKER_DEFAULT_PLATFORM must survive sudo, hence `sudo env ...`:
# the b/ builder's sudo shim execs its argv with the ambient env kept, but a
# real host sudo would strip a plain export.
runtime_platform_env=''
if [ -n "${TY_TARGET_ARCH:-}" ]; then
  echo "Will build the runtime images for linux/${TY_TARGET_ARCH}, because of TY_TARGET_ARCH."
  echo
  runtime_platform_env="DOCKER_DEFAULT_PLATFORM=linux/${TY_TARGET_ARCH}"
fi

# COMPOSE_PROFILES=backup: 'backup' is profile-gated, and compose does NOT
# apply DOCKER_DEFAULT_PLATFORM to a profile-disabled service that is merely
# named on the command line — it would silently build for the native arch.
# Enabling the profile makes the platform apply. (Harmless natively.)
function s_d_build_runtime() {
  sudo env $runtime_platform_env COMPOSE_PROFILES=backup docker compose build "$@"
}

# After a cross-arch build: fail loudly if any runtime image ended up the
# wrong architecture (e.g. a compose regression like the profiles+platform
# quirk above), instead of silently publishing a wrong-arch image.
function assert_runtime_images_arch() {
  if [ -z "${TY_TARGET_ARCH:-}" ]; then return 0; fi
  local reg_org=$(sed -nr 's/DOCKER_REG_ORG=([a-zA-Z0-9\._-]*).*/\1/p' .env)
  local svc arch
  for svc in rendr web cache rdb search egressp backup; do
    arch=$(sudo docker image inspect -f '{{.Architecture}}' "$reg_org/talkyard-$svc:latest")
    if [ "$arch" != "$TY_TARGET_ARCH" ]; then
      echo "Image $reg_org/talkyard-$svc:latest is '$arch', expected '$TY_TARGET_ARCH' — cross-arch build bug."
      die_if_in_script
    fi
  done
}


# Build Docker images
# ----------------------

if [ -z "$skip_build" ]; then
  # Build to-talkyard, needed in e2e tests.
  pushd .
  cd to-talkyard
  # For this, pnpm needs to be installed. [build_needs_yarn] Nix-shell is a good way
  # to get Nodejs and pnpm.
  pnpm build
  popd

  # Build minified script bundles; will be included in the web and app images. [PRODBNDLS]
  s/d build nodejs # so we'll use an up-to-date image when compiling Typescript
  s/d-gulp clean
  s/d-gulp build_release_dont_clean_before

  # Build images (except for the app server prod image). This will Dockerfile COPY
  # js and css generated by 'gulp build_...' above into 'web'.
  s/d build app # dev build, so we'll use the correct Ty image when running `test dist`.
  # One compose call, so Buildkit builds them in parallel (the web OpenResty
  # compile dominates). Always NATIVE here: the sbt step below starts
  # rendr/cache/rdb/search/egressp from these :latest tags (the dev app
  # service depends_on them), so they must not be cross-arch yet — the
  # cross-arch rebuild happens after the sbt step.
  sudo env COMPOSE_PROFILES=backup docker compose build rendr web cache rdb search egressp backup

  # Build the app server prod image.
  # First run tests though. All this needs lots of memory.
  # Then run `dist` to build the JAR file with Talkyard's compiled Scala code, which
  # we'll copy into the prod image.
  reuse_dist=''
  if [ -n "${TY_REUSE_APP_DIST:-}" ] && ls target/universal/talkyard-server-*.zip 1>/dev/null 2>&1; then
    # A second-arch pass right after a native build: the dist zip and the
    # unit test results are arch-independent, no need to redo them — but
    # only if the zip really is from THIS commit (a same-version zip from
    # an older commit would silently ship stale server code).
    if [ "$(cat target/universal/DIST_GIT_REV 2>/dev/null)" = "$(git rev-parse HEAD)" ]; then
      reuse_dist=yes
      echo "Reusing the app dist, because of TY_REUSE_APP_DIST:"
      ls target/universal/talkyard-server-*.zip
    else
      echo "TY_REUSE_APP_DIST set, but the dist zip is from another commit"
      echo "($(cat target/universal/DIST_GIT_REV 2>/dev/null || echo '?') != HEAD $(git rev-parse HEAD)) — rebuilding it."
    fi
  fi
  if [ -z "$reuse_dist" ]; then
    PLAY_HEAP_MEMORY_MB=8192 IS_PROD_TEST=true s/d-cli clean compile test:compile
    PLAY_HEAP_MEMORY_MB=8192 IS_PROD_TEST=true s/d-cli test dist
    git rev-parse HEAD > target/universal/DIST_GIT_REV
  fi
  s/d kill web app
  s/d down
  if [ -n "${TY_TARGET_ARCH:-}" ]; then
    # Only now — after sbt is done with the native dependency containers —
    # rebuild the runtime images cross-arch. (This overwrites the :latest
    # tags the sbt step's depends_on services were running from.)
    s_d_build_runtime rendr web cache rdb search egressp backup
    assert_runtime_images_arch
  fi
  # This will use the prod package built with 'dist' above.
  s/impl/build-prod-app-image.sh
fi


# Run End-to-End tests
# ----------------------

if [ -z "$skip_e2e_tests" ]; then

  # If there's a development Docker network, there'll be an IP address space clash
  # when creating a prodution test network later below. Delete any dev network.
  set +e
  sudo docker network rm tyd_internal_net
  set -e

  # Run the 'latest' tag — it's for the images we just built above.
  # '-p tyb1' = project Talkyard build, epoch 1.
  # Use the -no-limits.yml file, because we'll run performance tests.
  latest_test_containers=" \
      VERSION_TAG=latest \
      POSTGRES_PASSWORD=public \
      DOCKER_REG_ORG=debiki \
      $test_containers"

  if [ -z "$skip_restart"  -o  -z "`containers_running_test`" ]; then
    sudo $latest_test_containers down --volumes

    echo
    echo '------------------------------------------------------------------'
    echo "Starting e2e test servers:"
    echo "    $latest_test_containers up -d"   # _dupl_start_line
    echo '------------------------------------------------------------------'
    echo

    # Auto creates volumes:
    sudo $latest_test_containers up -d  # _dupl_start_line
  fi

  if [ -n "`jobs`" ]; then
    echo 'Other jobs running:'
    jobs
    echo 'Please stop them.'
    die_if_in_script
  fi


  exit_code_file='./target/e2e-tests-exit-code'
  rm -f $exit_code_file

  echo
  echo "Running Webdrier.io E2E tests ..."
  echo

  # Let us capture the exit code (instead of aborting on error exit code).
  set +e

  s/run-e2e-tests.sh --prod $all_orig_options

  # Capture e2e tests exit code.
  echo $? | tee $exit_code_file

  set -e

  e2e_tests_exit_code=$(cat $exit_code_file)

  if [ "$e2e_tests_exit_code" != '0' ]; then
    echo
    echo "E2E tests failed. Aborting build."
    echo
    echo "You can rerun the failed test — see above. The Talkyard server is still running."
    echo
    # These might be pretty long lines, so add blank lines in between.
    echo "To stop the server:"
    echo "   $latest_test_containers down"
    echo
    echo "To start it again:"
    echo "    $latest_test_containers up -d"
    echo
    die_if_in_script
  fi

  sudo $test_containers kill web app
  sudo $test_containers down
fi


# # Performance tests
# # -----------------
# 
# # (The perf test repo is currenty private)
# 
# pushd .
# cd ../ed-perf-test/
# ./test-performance.sh
# perf_test_result=$?
# 
# popd
# 
# if [ $perf_test_result -ne 0 ]; then
#   die_if_in_script
# fi



# # Rate & bandwidth limit tests
# # ----------------------
# 
# # Start the containers, but *with* rate limits this time.
# sudo $test_containers kill web
# sudo $test_containers down
# 
# # Run tests ... ensure gets 503 Service Unavailable ...
# # To do ...



# All done
# ----------------------

# If any Docker container is running now, something is amiss.
if [ -n "`containers_running_test`" ]; then
  echo
  echo "Some Docker stuff is still running. Why? Weird. Aborting. Look:"
  echo
  sudo docker ps
  echo
  die_if_in_script
fi

echo
echo "Done building and testing $version_tag."
echo "BUILD_OK" | tee ./target/build-exit-status


# vim: et ts=2 sw=2 tw=0 list
